...
def plot_3d_recons(originals, recons, epoch, recon_folder):
    ...
    for s in range(subjects_to_show):
        ...
        plot_slices(plot_slices(plot_slices(0, current_input, 'Original'), current_recon, 'Reconstruction'),
                    current_diff, 'Absolute difference')

        fig.tight_layout()
        plt.savefig(os.path.join(recon_folder, 'subject_' + str(s+1) + '.png'))

        prog_bar_counter += 1
        progress_bar(prog_bar_counter, subjects_to_show, prefix='Plotting 3D reconstructions:')
...
def plot_one_slice(data_to_plot, title=None, rot90=True, normalise=True, vmin=None, vmax=None):
...
def plot_3d_recons_v2(data_to_plot, titles, epoch, recon_folder, subjects_to_show=5, hyper_params=None, prefix='',
                      postfix='',
                      per_subject_prefix=None, suppress_progress_bar=False, normalise=True, vmin=None, vmax=None,
                      slices_per_axis=3, normalise_per_plot_type=None, vmin_vmax_per_plot_type=None):
    '''
    ...
    for i in range(subjects_to_show):
        ...
        for j in range(plot_types):
            ...
            if normalise or (normalise_per_plot_type is not None and normalise_per_plot_type[j]):
                do_normalise = True
                vmin = 0
                vmax = 1
            else:
            ...
            if vmin_vmax_per_plot_type is not None and vmin_vmax_per_plot_type[j] is not None:
            ...
            for k in range(1, slices_per_axis+1):
                plt.subplot(plot_types, (3 * slices_per_axis), j * (3 * slices_per_axis) + 0 * slices_per_axis + k)
                plot_one_slice(current[k*axis_length, :, :], normalise=do_normalise, vmin=current_vmin, vmax=current_vmax)

            axis_length = current.shape[1]
            axis_length = axis_length // (slices_per_axis + 1)
            for k in range(1, slices_per_axis+1):
                if k == np.ceil(slices_per_axis / 2):
                    title = titles[j]  # Put the title immediately above the middle item!
                else:
                    title = None
                plt.subplot(plot_types, (3 * slices_per_axis), j * (3 * slices_per_axis) + 1 * slices_per_axis + k)
                plot_one_slice(current[:, k*axis_length, :], title=title, normalise=do_normalise, vmin=current_vmin, vmax=current_vmax)

            axis_length = current.shape[2]
            axis_length = axis_length // (slices_per_axis + 1)
            for k in range(1, slices_per_axis+1):
                plt.subplot(plot_types, (3 * slices_per_axis), j * (3 * slices_per_axis) + 2 * slices_per_axis + k)
                plot_one_slice(current[:, :, k*axis_length], normalise=do_normalise, vmin=current_vmin, vmax=current_vmax)

        if per_subject_prefix:
            plt.savefig(os.path.join(recon_folder[i], prefix + per_subject_prefix[i] + postfix + '.png'))
        else:
            plt.savefig(os.path.join(recon_folder[i], prefix + '_subject_' + str(i+1) + postfix + '.png'))

        if not suppress_progress_bar and subjects_to_show > 1:
            progress_bar(i+1, subjects_to_show, prefix='Plotting:')


def plot_error_curves(data, labels, plot_title, recon_folder, postifx='', prefix='', precision=6, xlabel=None, ylabel=None):
    ...
    for k in range(num_plots):
        ...
        #          ls=current_style)
        plt.plot(current_data[:, 0], current_data[:, 1], color=current_colour, label=current_label + appendage,
                 ls=current_style)
        # plt.plot(current_data[:, 0], current_data[:, 1], 'o-', color=current_colour, label=current_label + appendage)
...
def plot_2d_samples(samples, epoch, recon_folder, is_colour=False):
    ...
    for i in range(side_length):
        for j in range(side_length):
            ...
            current_input = visuals.norm_zero_to_one(current_input)
            if is_colour:
...
def image_grid(data_to_plot, epoch, recon_folder, filename='samples', num_to_plot=5, norm_recons=False):
    """
    Given a numpy array of shape batch x 1 x h x w, this function plots a grid of images
    """

    side_length = int(np.max([1, np.min([num_to_plot, np.floor(np.sqrt(0.5 * data_to_plot.shape[0]))])]))
    ...
    if side_length == 1:
        grid = ImageGrid(fig=fig, rect=111, nrows_ncols=(side_length, side_length), axes_pad=0.05)
    else:
        grid = ImageGrid(fig=fig, rect=111, nrows_ncols=(side_length, 2 * side_length), axes_pad=0.05)
    ...
    for s in range(subjects_to_show):
        ...
        for axis in range(3):
            for slice_number in range(3):
                ...
                plt.imshow(current_slice, cmap='gray')
                plt.axis('off')
                if axis == slice_number == 1:  # This slice is in the middle of the row of slices
...
def plot_2d(data_to_plot, titles, epoch, recon_folder, filename='samples', is_colour=False, num_to_plot=5,
            norm_recons=False):
    """
    ...
    for k in range(num_to_plot_here):
        counter += 1
        for j in range(plot_types):
            plt.subplot(num_to_plot_here, plot_types, plot_types * counter - (plot_types - j - 1))
            if counter == 1:
                plt.title(titles[j])
            plt.axis('off')
            ...
            if is_colour:
                ...
                if norm_recons:
                    plt.imshow(current, cmap='gray', vmin=0, vmax=1, interpolation='none')
                else:
                    plt.imshow(current)
