...
from unpooling_block import UnPoolingBlock
from top_down_block_simplified_translator_v2 import TopDownBlock
import misc
from tanh_block import TanhBlock
from sigmoid_block import SigmoidBlock
from tanh_first_n_channels_block import TanhFirstNChansOnlyBlock
from sigmoid_first_n_channels_block import SigmoidFirstNChansOnlyBlock
...
class Graph:
    ...
    def __init__(self, **kwargs):
        ...
        self.unpool = UnPoolingBlock(hyper_params=hyper_params, half_precision=hyper_params['half_precision'])
        ...
        if 'hidden_spatial_dims' in hyper_params:
            ...
            for size in hyper_params['hidden_spatial_dims'][0:-1][::-1]:
                ...
                self.unpooling_ops.append(UnPoolingBlock(hyper_params=hyper_params,
        ...
        self.weight_sharing_index = hyper_params['latents_per_channel_weight_sharing'][::-1]
        ...
        latent_counter = 0
        current_block = []
        current_block += [TopDownBlock(non_imaging_dims=non_imaging_dims,  # Either concat this many zeros to the flat latent, or concat the non-imaging data if it's in the data dictionary
        ...
        current_block += [ConvBlock(channels_in=conv_block_channels_in,
        ...
        for n in range(1, latents_to_use):
            latent_counter += 1
            if self.latents_to_use is not None and not self.latents_to_use[-1-latent_counter]:
                # Replace TopDown with ConvBlock
                current_block += [SkipBlock(channels_in=channels_out,
                                            channels_out=channels_out,
                                            hyper_params=hyper_params,
                                            half_precision=hyper_params['half_precision'])]
            else:
        ...
        for k in range(len(channels)-1):
            ...
            if latents_to_use == 0:
                ...
                current_block += [ConvBlock(channels_in=conv_block_channels_in,
                                            ...
                                            channels_out=channels_out,
                                            hidden_kernel_size=hidden_kernel_size,
                                            veto_bottleneck='do_not_use_bottleneck_in_last_block' in hyper_params and hyper_params['do_not_use_bottleneck_in_last_block'] and last_layer,  # Avoid creating a bottleneck
                                            hyper_params=hyper_params,
                                            normalise_weight_by_depth=True,
                ...
                if 'hidden_spatial_dims' in hyper_params:
                    ...
                    current_block = [self.unpool]
                if self.latents_to_use is not None and not self.latents_to_use[-1 - latent_counter]:
                    current_block += [SkipBlock(channels_in=channels_in,
                                                channels_out=channels_out,
                                                hyper_params=hyper_params,
                                                half_precision=hyper_params['half_precision'])]
                else:
                    ...
                    current_block += [TopDownBlock(length_of_flag=length_of_flag,
                                                   conditional_prior=self.conditional_prior,
                                                   channels_in=channels_in,
                                                   channels_hidden=channels_hid,
                                                   channels_out=channels_out,
                ...
                for n in range(1, latents_to_use):
                    ...
                    if self.latents_to_use is not None and not self.latents_to_use[-1 - latent_counter]:
                        ...
                        current_block += [TopDownBlock(length_of_flag=length_of_flag,  # Specified in block 0
                                                       conditional_prior=self.conditional_prior,
                                                       channels_in=channels_out,
                                                       channels_hidden=channels_hid,
                                                       channels_out=channels_out,
        ...
        if hyper_params['predict_x_var'] and hyper_params['separate_output_loc_scale_convs']:
            # Need a block to produce mean and log_var of p(x|z).
            # Channel 0 of the output is the mean, 1 is the variance
            block = []
            block += [ConvBlock(channels_in=channels[-1],
            ...
            elif 'use_sigmoid_output' in hyper_params and hyper_params['use_sigmoid_output']:
            ...
            if 'experimental_activate_chans_0_1_only' in hyper_params and \
                hyper_params['experimental_activate_chans_0_1_only']:
                experimental_activate_chans_0_1_only = hyper_params['experimental_activate_chans_0_1_only']
            else:
            ...
            if experimental_activate_chans_0_1_only:
                if 'use_tanh_output' in hyper_params and hyper_params['use_tanh_output']:
                    if hyper_params['predict_x_var']:
                        out_trans = TanhFirstNChansOnlyBlock(half_precision=hyper_params['half_precision'],
                    ...
                    if hyper_params['predict_x_var']:
                        out_trans = SigmoidFirstNChansOnlyBlock(half_precision=hyper_params['half_precision'],
                ...
                if 'use_tanh_output' in hyper_params and hyper_params['use_tanh_output']:
                    if hyper_params['predict_x_var']:
                        ...
                        out_trans = TanhBlock(half_precision=hyper_params['half_precision'])
                    ...
                    if hyper_params['predict_x_var']:
                        ...
                        out_trans = SigmoidBlock(half_precision=hyper_params['half_precision'])
