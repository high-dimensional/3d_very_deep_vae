...
class PoolingBlock(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        if 'channels' in self.kwargs:
            ...
            self.resampler = ConvOp(in_channels=self.channels, out_channels=self.channels, kernel_size=2, stride=2,
                                    padding=0, bias=True)
        else:
    ...
    def forward(self, input_dictionary):
        with amp.autocast(enabled=self.kwargs["half_precision"]):
            ...
            if 'channels' in self.kwargs:
                ...
                input_dictionary['data'] = nn.functional.interpolate(input_dictionary['data'],
                                                                     scale_factor=0.5,
