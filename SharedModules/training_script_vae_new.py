...
import platform
import socket
from TranslatorGraph import Graph as Graph
from dataset_creator import DatasetCreator
import data_handling
import loss_functions
import visuals
import misc
import run_epoch_hemispheres as run_epoch
...
from hdf5_data_loader import HDF5Loader
...
from data_transformations import create_data_transformations


random_seed = 42
# torch.manual_seed(random_seed)  # Workers end up with same seed
...
def main(hyper_params):
    ...
    # os.environ['MASTER_ADDR'] = str(hyper_params['master_address'])
    # os.environ['MASTER_PORT'] = str(hyper_params['port'])
    # os.environ['OMP_NUM_THREADS'] = str(hyper_params['workers_per_process'])
    #
    # if 'local_rank' in hyper_params:
    #     local_rank = hyper_params['local_rank']
    #     hyper_params['local_rank'] = int(local_rank)
    # else:
    ...
    #
    # if hasattr(hyper_params['args'], 'nnodes'):
    #     world_size = int(hyper_params['args'].nnodes * hyper_params['args'].nproc_per_node)
    #     misc.print_0(hyper_params, "Found world_size in input argument nproc_per_node: " + str(world_size))
    # elif hasattr(hyper_params['args'], 'nproc_per_node'):
    #     world_size = hyper_params['args'].nproc_per_node
    #     misc.print_0(hyper_params, "Found world_size in input argument nproc_per_node: " + str(world_size))
    # elif 'world_size' in hyper_params:
    #     world_size = hyper_params['world_size']
    #     misc.print_0(hyper_params, "Found world_size in hyper_params: " + str(world_size))
    # elif hasattr(hyper_params['args'], 'world_size'):
    #     world_size = hyper_params['args'].world_size
    #     misc.print_0(hyper_params, "Found world_size in input argument world_size: " + str(world_size))
    # else:
    #     world_size = len(hyper_params['CUDA_devices'])
    #     misc.print_0(hyper_params, "Found world_size in CUDA_devices: " + str(world_size))
    #
    ...
    # misc.print_0(hyper_params, "Host IP: " + str(hyper_params['master_address']))
    # misc.print_0(hyper_params, "Host Port: " + str(hyper_params['port']))
