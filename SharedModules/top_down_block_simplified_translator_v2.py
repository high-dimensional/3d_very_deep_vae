...
class TopDownBlock(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        if 'minimum_upper_hidden_bound' in self.hyper_params:
            ...
            self.biggest_latent_index = np.sum(self.hyper_params['latents_per_channel']) - 1
            self.minimum_upper_variance_bound = self.hyper_params['minimum_upper_hidden_bound']
            if 'data_is_3d' in self.hyper_params and self.hyper_params['data_is_3d']:
                self.variance_bounds[1] = self.variance_bounds[1] * ((
                                                                                 self.biggest_latent_index - self.index_of_latent) / self.biggest_latent_index) ** 3 + self.minimum_upper_variance_bound * (
                                                      self.index_of_latent / self.biggest_latent_index) ** 3
            else:
                self.variance_bounds[1] = self.variance_bounds[1] * ((
                                                                                 self.biggest_latent_index - self.index_of_latent) / self.biggest_latent_index) ** 2 + self.minimum_upper_variance_bound * (
        ...
        if 'channels_for_latent' in self.kwargs and self.kwargs["channels_for_latent"] < self.channels_out:
        ...
        if 'data_is_3d' in self.hyper_params and self.hyper_params['data_is_3d']:
            ...
            d = 3
            filter_space_dims = str(self.hidden_kernel_size) + "x" + str(self.hidden_kernel_size) + "x" + str(
        ...
        if 'uppermost_block' in self.kwargs and self.kwargs["uppermost_block"]:
            ...
            if 'veto_skip_con' in self.kwargs and self.kwargs["veto_skip_con"]:
                ...
                if hasattr(self.shared_group_ops, 'skip_con_conv'):
                    ...
                    if not self.channels_in == self.channels_out:
                        self.skip_con_conv = ConvOp(in_channels=self.channels_in, out_channels=self.channels_out,
                                                    kernel_size=1, stride=1, padding=0, bias=True)

                        self.param_count += self.channels_in * self.channels_out
                        out_txt += "> skip: " + one_dims + "x" + str(self.channels_in) + "x" + str(self.channels_out) \
                                   + "\n"
        ...
        if self.uppermost_block:
            ...
            if self.conditional_model:
                ...
                dims_into_prior_predicter = self.lateral_connection_channels + self.channels_in
            else:
                dims_into_prior_predicter = self.channels_in
        ...
        if self.separate_loc_scale_convs:
            ...
            if hasattr(self.shared_group_ops, 'convs_p_mu') and hasattr(self.shared_group_ops, 'convs_p_log_var'):
                self.convs_p_mu = self.shared_group_ops.convs_p_mu
                self.convs_p_log_var = self.shared_group_ops.convs_p_log_var
            else:
                if not (
                        self.uppermost_block and not self.conditional_model):  # Prior not predicted from data in uppermost block when not conditional model
                    ...
                    self.convs_p_mu.append(
                        ConvOp(in_channels=dims_into_prior_predicter, out_channels=self.channels_hidden,
                               kernel_size=1, stride=1, padding=0, bias=True))
                    self.convs_p_mu.append(ConvOp(in_channels=self.channels_hidden, out_channels=self.channels_hidden,
                                                  kernel_size=self.hidden_kernel_size, stride=1, padding=0, bias=True))
                    self.convs_p_mu.append(ConvOp(in_channels=self.channels_hidden, out_channels=self.channels_hidden,
                                                  kernel_size=self.hidden_kernel_size, stride=1, padding=0, bias=True))
                    self.convs_p_mu.append(
                        ConvOp(in_channels=self.channels_hidden, out_channels=self.channels_for_latent,
                               kernel_size=1, stride=1, padding=0, bias=True))
                    ...
                    self.convs_p_log_var.append(
                        ConvOp(in_channels=dims_into_prior_predicter, out_channels=self.channels_hidden,
                               kernel_size=1, stride=1, padding=0, bias=True))
                    self.convs_p_log_var.append(
                        ConvOp(in_channels=self.channels_hidden, out_channels=self.channels_hidden,
                               kernel_size=self.hidden_kernel_size, stride=1, padding=0,
                               bias=True))
                    self.convs_p_log_var.append(
                        ConvOp(in_channels=self.channels_hidden, out_channels=self.channels_hidden,
                    ...
                    self.convs_p_mu[-1].weight.data *= 0.
                    self.convs_p_log_var[-1].weight.data *= 0.

                    self.param_count += 2 * dims_into_prior_predicter * self.channels_hidden
                    self.param_count += 4 * self.hidden_kernel_size ** d * self.channels_hidden * self.channels_hidden
                    self.param_count += 2 * self.channels_hidden * self.channels_for_latent

                    out_txt += "> p(z): " + one_dims + "x" + str(dims_into_prior_predicter) + "x" + str(
                        self.channels_hidden)
                    out_txt += ", " + filter_space_dims + "x" + str(self.channels_hidden) + "x" + str(
                        self.channels_hidden)
                    out_txt += ", " + filter_space_dims + "x" + str(self.channels_hidden) + "x" + str(
                        self.channels_hidden)
                    out_txt += ", " + one_dims + "x" + str(self.channels_hidden) + "x" + str(self.channels_for_latent)

                    out_txt += "; " + one_dims + "x" + str(dims_into_prior_predicter) + "x" + str(self.channels_hidden)
                    out_txt += ", " + filter_space_dims + "x" + str(self.channels_hidden) + "x" + str(
                        self.channels_hidden)
                    out_txt += ", " + filter_space_dims + "x" + str(self.channels_hidden) + "x" + str(
                        self.channels_hidden)
                    out_txt += ", " + one_dims + "x" + str(self.channels_hidden) + "x" + str(
                        self.channels_for_latent) + "\n"
            ...
            if hasattr(self.shared_group_ops, 'convs_p'):
                ...
                if not (self.uppermost_block and not self.conditional_model):  # Prior not predicted from data in uppermost block when not conditional model
                    ...
                    self.convs_p[-1].weight.data *= 0.
                    self.convs_p[-2].weight.data *= 0.

                    self.param_count += dims_into_prior_predicter * self.channels_hidden
                    self.param_count += 2 * self.hidden_kernel_size ** d * self.channels_hidden * self.channels_hidden
                    self.param_count += 2 * self.channels_hidden * self.channels_for_latent
                    out_txt += "> p(z): " + one_dims + "x" + str(dims_into_prior_predicter) + "x" + str(
                        self.channels_hidden)
                    out_txt += ", " + filter_space_dims + "x" + str(self.channels_hidden) + "x" + str(
        ...
        if self.conditional_model or not self.uppermost_block:
            ...
            self.convs_p_skip_conv_after_prior = ConvOp(in_channels=self.channels_hidden,
                                                        out_channels=self.channels_out,
                                                        kernel_size=1, stride=1, padding=0, bias=True)

            if 'zero_biases' in self.hyper_params and self.hyper_params['zero_biases']:
                self.convs_p_skip_conv_after_prior.weight.data *= 0.  # In the v deep VAE paper
                self.convs_p_skip_conv_after_prior.bias.data *= 0.

            out_txt += "> skip_p: " + one_dims + "x" + str(self.channels_hidden) + "x" + str(
                self.channels_out) + "\n"
        ...
        if self.uppermost_block:
            ...
            # Predict posterior from lateral skip connection AND incoming data from previous block
            dims_into_posterior_predicter = self.lateral_connection_channels + self.channels_in

        dims_into_posterior_predicter += self.length_of_flag
        dims_into_posterior_predicter += self.non_imaging_dims

        if self.separate_loc_scale_convs:
            if hasattr(self.shared_group_ops, 'convs_q_mu') and hasattr(self.shared_group_ops, 'convs_q_log_var'):
                ...
                self.convs_q_mu.append(
                    ConvOp(in_channels=dims_into_posterior_predicter, out_channels=self.channels_hidden,
                           kernel_size=1, stride=1, padding=0, bias=True))
                self.convs_q_mu.append(ConvOp(in_channels=self.channels_hidden, out_channels=self.channels_hidden,
                                              kernel_size=self.hidden_kernel_size, stride=1, padding=0, bias=True))
                self.convs_q_mu.append(ConvOp(in_channels=self.channels_hidden, out_channels=self.channels_hidden,
                                              kernel_size=self.hidden_kernel_size, stride=1, padding=0, bias=True))
                self.convs_q_mu.append(ConvOp(in_channels=self.channels_hidden, out_channels=self.channels_for_latent,
                ...
                out_txt += "> q(z): " + one_dims + "x" + str(dims_into_prior_predicter) + "x" + str(
                    self.channels_hidden)
                out_txt += ", " + filter_space_dims + "x" + str(self.channels_hidden) + "x" + str(self.channels_hidden)
                out_txt += ", " + filter_space_dims + "x" + str(self.channels_hidden) + "x" + str(self.channels_hidden)
                out_txt += ", " + one_dims + "x" + str(self.channels_hidden) + "x" + str(self.channels_for_latent)
            ...
            if hasattr(self.shared_group_ops, 'convs_q'):
                ...
                out_txt += "> q(z): " + one_dims + "x" + str(dims_into_posterior_predicter) + "x" + str(
                    self.channels_hidden)
                out_txt += ", " + filter_space_dims + "x" + str(self.channels_hidden) + "x" + str(self.channels_hidden)
                out_txt += ", " + filter_space_dims + "x" + str(self.channels_hidden) + "x" + str(self.channels_hidden)
                out_txt += ", " + one_dims + "x" + str(self.channels_hidden) + "x" + str(self.channels_for_latent)
        ...
        if hasattr(self.shared_group_ops, 'conv_z'):
            ...
            self.conv_z = ConvOp(in_channels=self.channels_for_latent + self.length_of_flag + self.non_imaging_dims,
                                 out_channels=self.channels_out,
                                 kernel_size=1, stride=1, padding=0, bias=True)

            if 'zero_biases' in self.hyper_params and self.hyper_params['zero_biases']:
                self.conv_z.bias.data *= 0.
            ...
            out_txt += "> conv_z: " + one_dims + "x" + str(self.channels_for_latent) + "x" + str(self.channels_out)

        if 'verbose' in self.hyper_params and self.hyper_params['verbose']:
            print("Top-down block kernels (" + str(self.param_count) + "): " + "\n" + out_txt)
    ...
    def forward(self, data_dictionary):
        with amp.autocast(enabled=self.kwargs["half_precision"]):
            ...
            copy_of_incoming_data = data_dictionary['data'].clone()  # For skip connections
            ...
            if is_sampling:
                ...
                if self.uppermost_block:
                    ...
                    copy_of_incoming_data = torch.zeros_like(data_dictionary['data'])  # Just to be sure
                else:
                    if self.conditional_model:
                        if data_dictionary['encoder2_data'] is None:
                            ...
                            shape = [batch_size,
                                     self.hyper_params['channels'][self.lateral_skip_con_to_use]] + spatial_dims
                            device = data_dictionary['data'].device
                            from_bottom_up_2 = torch.zeros(shape, device=device, dtype=torch.float32)
                        else:
                            from_bottom_up_2 = data_dictionary[
                                'encoder2_lateral_skip_' + str(self.lateral_skip_con_to_use)]
                        p_data = torch.cat((from_bottom_up_2, data_dictionary['data']), dim=1)
                    else:
                ...
                if self.non_imaging_dims > 0:
                    ...
                    if 'non_imaging_data' in data_dictionary:
                        ...
                        if len(p_data.shape) == 4:
                            non_imaging_data = torch.zeros((batch_size, self.non_imaging_dims, 1, 1), device=device)
                        else:
                            non_imaging_data = torch.zeros((batch_size, self.non_imaging_dims, 1, 1, 1), device=device)
                ...
                if self.uppermost_block and not self.conditional_model:
                    ...
                    data = torch.randn(size=latent_shape, device=data_dictionary['data'].device)
                    ...
                    if self.non_imaging_dims > 0:
                        ...
                        if 'non_imaging_data' in data_dictionary:
                            ...
                            device = data.device
                            if len(data.shape) == 4:
                                non_imaging_data = torch.zeros((batch_size, self.non_imaging_dims, 1, 1),
                                                               device=device)
                            else:
                                non_imaging_data = torch.zeros((batch_size, self.non_imaging_dims, 1, 1, 1),
                                                               device=device)
                    ...
                    if misc.key_is_true(self.hyper_params, 'predict_x_var_with_sigmoid'):
                        ...
                        if self.variance_bounds is not None:
                            p_log_var = torch.clamp(p_log_var, self.variance_bounds[0], self.variance_bounds[1])

                        # Sample z
                        data = p_mu + torch.mul(torch.exp(0.5 * p_log_var), torch.randn_like(p_log_var))
                ...
                Training mode
                """
                from_bottom_up_1 = data_dictionary['encoder1_lateral_skip_' + str(self.lateral_skip_con_to_use)]
                if self.conditional_model:
                    if data_dictionary['encoder2_data'] is None:
                        """
                        ...
                        batch_size = data_dictionary['data'].shape[0]
                        spatial_dims = list(data_dictionary['data'].shape[2:])
                        shape = [batch_size, self.hyper_params['channels'][self.lateral_skip_con_to_use]] + spatial_dims
                        device = data_dictionary['data'].device
                        from_bottom_up_2 = torch.zeros(shape, device=device, dtype=torch.float32)
                    else:
                        from_bottom_up_2 = data_dictionary['encoder2_lateral_skip_' + str(self.lateral_skip_con_to_use)]
                ...
                if self.uppermost_block:
                    if self.conditional_model:
                        ...
                        if data_dictionary['encoder2_data'] is None:
                            ...
                            device = data_dictionary['data'].device
                            batch_size = data_dictionary['data'].shape[0]
                            channels = self.hyper_params['channels'][self.lateral_skip_con_to_use]
                            spatial_dims = list(data_dictionary['data'].shape[2:])
                            shape_skip = [batch_size, channels] + spatial_dims
                            p_skip = torch.zeros(shape_skip, device=device, dtype=torch.float32)

                            channels = self.hyper_params['channels_per_latent'][self.lateral_skip_con_to_use]
                            shape = [batch_size, channels] + spatial_dims
                            p_mu = torch.zeros(shape, device=device, dtype=torch.float32)
                            p_log_var = torch.zeros(shape, device=device, dtype=torch.float32)

                            if misc.key_is_true(self.hyper_params, 'predict_x_var_with_sigmoid'):
                                p_std = torch.ones(shape, device=device, dtype=torch.float32)
                    ...
                    q_data = torch.cat((from_bottom_up_1, data_dictionary['data']), dim=1)
                ...
                if misc.key_is_true(self.hyper_params, 'predict_x_var_with_sigmoid'):
                    ...
                    if misc.key_is_true(self.hyper_params, 'model_posterior_as_perturbation_of_prior'):
                        if not self.uppermost_block:
                            ...
                            if 'test_op' in self.hyper_params:
                                delta_log_var = torch.clamp(delta_log_var, -self.hyper_params['test_op'],
                                                            self.hyper_params['test_op'])
                                delta_mu = torch.clamp(delta_mu, -self.hyper_params['test_op'],
                                                       self.hyper_params['test_op'])
                    ...
                    if self.variance_bounds is not None:
                        q_log_var = torch.clamp(q_log_var, self.variance_bounds[0], self.variance_bounds[1])
                ...
                if self.inject_noise:
                    ...
                    if 'noise_injection_multiplier' in self.hyper_params and not self.hyper_params[
                                                                                     'noise_injection_multiplier'] == 1:
                ...
                if not ('veto_KL' in data_dictionary and data_dictionary['veto_KL']):
                    ...
                    if misc.key_is_true(self.hyper_params, 'predict_x_var_with_sigmoid'):
                        if self.uppermost_block and not self.conditional_model:
                            data_dictionary['KL_list'].append(misc.kl_stds(q_mu, q_std, None, None, kl_mask))
                        else:
                            data_dictionary['KL_list'].append(misc.kl_stds(q_mu, q_std, p_mu, p_std, kl_mask))
                    else:
                        if self.uppermost_block and not self.conditional_model:
                            data_dictionary['KL_list'].append(misc.kl_log_vars(q_mu, q_log_var, None, None))
                        else:
                            if misc.key_is_true(self.hyper_params, 'model_posterior_as_perturbation_of_prior'):
                                data_dictionary['KL_list'].append(
                                    misc.kl_perturbed_prior(delta_mu, delta_log_var, p_log_var))
                            else:
                                data_dictionary['KL_list'].append(misc.kl_log_vars(q_mu, q_log_var, p_mu, p_log_var))
