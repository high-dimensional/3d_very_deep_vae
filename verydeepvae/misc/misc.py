...
def count_gradient_nans(
    ...
    grad_norm_bu1 = torch.nn.utils.clip_grad_norm_(
        bottom_up_graph_1.model.parameters(), c
    ).item()
    gradient_norms["bottom_up_graph_1"].append([iteration, grad_norm_bu1])
    grad_norm_td1 = torch.nn.utils.clip_grad_norm_(
        top_down_graph.latents.parameters(), c
    ).item()
    gradient_norms["top_down_graph_latents"].append([iteration, grad_norm_td1])
    grad_norm_td2 = torch.nn.utils.clip_grad_norm_(
        top_down_graph.x_mu.parameters(), c
    ).item()
    ...
    if log_likelihood is None:
        ...
        nan_count_loglikelihood = (
            torch.isnan(log_likelihood).sum() + torch.isinf(log_likelihood).sum()
        )
...
def gaussian_likelihood(batch_target_features, x_mu, x_var, x_log_var, hyper_params):
    ...
    if hyper_params["predict_x_var"]:
        ...
        if (
            "kl_multiplier" in hyper_params
            and not hyper_params["kl_multiplier"] == 1
            and not hyper_params["kl_multiplier"] == 0
        ):
            a = hyper_params["kl_multiplier"]
            log_likelihood_per_dim = -0.5 * (
                np.log(a) + np.log(2 * np.pi) + torch.true_divide(squared_difference, a)
            )
        else:
            log_likelihood_per_dim = -0.5 * (
                np.log(2 * np.pi) + squared_difference
            )  # Assuming x_var = I
...
def kl_stds(mu, std, mu_2=None, std_2=None, kl_mask=None):
    ...
    if mu_2 is None:
        ...
        squared_diff = torch.square(mu - mu_2)
        output = 0.5 * (
            torch.div(var + squared_diff, torch.maximum(var_2, eps))
            - 1
            + 2 * torch.log(std_2)
            - 2 * torch.log(std)
        )
