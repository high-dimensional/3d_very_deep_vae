...
def setup_environment(hyper_params):
    ...
    if "LOCAL_WORLD_SIZE" in os.environ and os.environ["LOCAL_WORLD_SIZE"] is not None:
        ...
        misc.print_0(
            hyper_params, "Found local_rank in LOCAL_RANK env var: " + str(local_rank)
        )

        global_rank = int(os.environ["RANK"])
        hyper_params["global_rank"] = global_rank
        misc.print_0(
            hyper_params, "Found global_rank in RANK env var: " + str(global_rank)
        )
        ...
        misc.print_0(
            hyper_params,
            "Found global_world_size in WORLD_SIZE env var: " + str(global_world_size),
        )
        ...
        misc.print_0(
            hyper_params,
            "Found world_size in LOCAL_WORLD_SIZE env var: " + str(world_size),
        )
        ...
        misc.print_0(
            hyper_params,
            "Found master_addr in MASTER_ADDR env var: " + str(master_addr),
        )
        ...
        misc.print_0(
            hyper_params,
            "Found master_port in MASTER_ADDR env var: " + str(master_port),
        )
    else:
        local_rank = hyper_params["local_rank"]
        misc.print_0(
            hyper_params, "Found local_rank in hyper_params: " + str(local_rank)
        )

        if "world_size" in hyper_params:
            world_size = hyper_params["world_size"]
            misc.print_0(
                hyper_params, "Found world_size in hyper_params: " + str(world_size)
            )
        else:
            hyper_params["world_size"] = len(hyper_params["CUDA_devices"])
            misc.print_0(
                hyper_params,
                "world_size not found in hyper_params, using len(CUDA_devices): "
    ...
    misc.print_0(
        hyper_params, f"Setting number of threads to {hyper_params['threads_per_rank']}"
    )
    ...
    misc.print_0(
        hyper_params, "Workers per process: " + str(hyper_params["workers_per_process"])
    )

    if hyper_params["global_world_size"] > 1:
        misc.print_0(
            hyper_params,
            "Waiting for all DDP processes to establish contact...",
            end="",
        )
    ...
    if "LOCAL_WORLD_SIZE" in os.environ:
        ...
        dist.init_process_group(
            backend=backend,
            rank=local_rank,
            world_size=hyper_params["global_world_size"],
        )
    ...
    misc.print_0(
        hyper_params,
        "Multiprocessing start method: " + torch.multiprocessing.get_start_method(),
    )
    ...
    misc.print_0(
        hyper_params, "CUDA_VISIBLE_DEVICES: " + os.environ["CUDA_VISIBLE_DEVICES"]
    )
    misc.print_0(hyper_params, "Backend: " + backend)
    hyper_params["samples_folder"] = os.path.join(
        hyper_params["recon_folder"], "samples"
    )
    ...
    if local_rank == 0:
        ...
        misc.print_0(
            hyper_params,
            "To start TB: tensorboard --logdir "
            + hyper_params["tensorboard_dir"]
            + " --port=8008",
        )
        if not (
            hyper_params["resume_from_checkpoint"] and hyper_params["restore_optimiser"]
        ):
            data_handling.delete_directory_contents(hyper_params["tensorboard_dir"])

    misc.print_0(
        hyper_params, "Checkpoint folder: " + hyper_params["checkpoint_folder"]
    )
