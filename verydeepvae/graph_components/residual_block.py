...
class ResBlock(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.convolutions.append(nn.Conv2d(in_channels=self.channels_in, out_channels=self.channels_hidden,
                                           kernel_size=1, stride=1, padding=0, bias=True))
        self.convolutions.append(nn.Conv2d(in_channels=self.channels_hidden, out_channels=self.channels_hidden,
                                           kernel_size=self.hidden_kernel_size, stride=1, padding=0, bias=True))
        self.convolutions.append(nn.Conv2d(in_channels=self.channels_hidden, out_channels=self.channels_hidden,
                                           kernel_size=self.hidden_kernel_size, stride=1, padding=0, bias=True))
        self.convolutions.append(nn.Conv2d(in_channels=self.channels_hidden, out_channels=self.channels_out,
                                           kernel_size=1, stride=1, padding=0, bias=True))

        # self.batch_norm = nn.SyncBatchNorm(num_features=self.channels_in, eps=1e-5, momentum=0.1, affine=True,
        #                                    track_running_stats=True)
        self.batch_norm = nn.BatchNorm2d(num_features=self.channels_in, eps=1e-5, momentum=0.1, affine=True,
        track_running_stats=True)

        if self.channels_in == self.channels_out:
            self.skip_con = nn.Identity()
        else:
            self.skip_con = nn.Conv2d(in_channels=self.channels_in, out_channels=self.channels_out,
                                      kernel_size=1, stride=1, padding=0, bias=True)
