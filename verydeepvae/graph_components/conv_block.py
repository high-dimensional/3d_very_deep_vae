    ...
    def __init__(self, **kwargs):
        ...
        if (
            ...
            if not self.channels_in == self.channels_out:
                ...
                if (
                    "zero_biases" in self.hyper_params
                    and self.hyper_params["zero_biases"]
                ):
        ...
        if self.veto_bottleneck:
            if (
                "output_block_setup" in self.kwargs
                and self.kwargs["output_block_setup"]
            ):
                self.convolutions = nn.ModuleList()
                self.convolutions.append(
                    ConvOp(
                        in_channels=self.channels_in,
                        out_channels=self.channels_in,
                ...
                if "verbose" in self.hyper_params and self.hyper_params["verbose"]:
                    ...
                    print(
                        "Conv block kernels (" + str(self.param_count) + "): " + out_txt
                    )
            elif (
                "output_block2_setup" in self.kwargs
                and self.kwargs["output_block2_setup"]
            ):
                self.convolutions = nn.ModuleList()
                self.convolutions.append(
                    ConvOp(
    ...
    def forward(self, data_dictionary):
        with amp.autocast(enabled=self.kwargs["half_precision"]):
            ...
            if self.concat_flag:
                ...
                posterior_input_channels = torch.zeros(
                    size=starting_shape,
                    dtype=data.dtype,
                    device=data.device,
                    requires_grad=False,
            ...
            if self.use_skip_connection:
                if (
                    ...
                    if self.channels_in == self.channels_out:
                        ...
                        data = self.rezero_alpha * data + self.skip_con(
                            copy_of_incoming_data
                        )
                else:
                    if self.channels_in == self.channels_out:
