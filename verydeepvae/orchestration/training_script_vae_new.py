...
def main(hyper_params):
    ...
    if hyper_params['use_nii_data']:
        ...
        val_transforms, train_transforms = create_data_transformations(hyper_params, hyper_params['device'])
    ...
    top_down_graph = TopDownGraph(hyper_params=hyper_params, device=hyper_params['device'])
    ...
    for epoch in range(starting_epoch, hyper_params['total_epochs']):
        ...
        if not hyper_params['validation_period'] == 1 and hyper_params['local_rank'] == 0:
            ...
            visuals.plot_error_curves(data=[loss_history_train_mse], labels=['training MSE'],
                                      ...
                                      prefix='mse_train')
            visuals.plot_error_curves(data=[loss_history_train_nll_bits_per_dim],
        ...
        if hyper_params['local_rank'] == 0 and 'plot_gradient_norms' in hyper_params and \
                hyper_params['plot_gradient_norms']:
                ...
                if hyper_params['local_rank'] == 0:
                    if len(loss_history_val_loss) == len(loss_history_train_loss):
                        ...
                        visuals.plot_error_curves(data=[loss_history_val_mse], labels=['validating MSE'],
                                                  ...
                                                  recon_folder=hyper_params['recon_folder'],
                                                  prefix='mse_val')
                        visuals.plot_error_curves(data=[loss_history_val_nll_bits_per_dim],
                        ...
                        for k in range(times_to_sample):
                            ...
                            if is_3d:
                                ...
                                for m in range(batch_shape[0]):
                                    current_filename = 'rank' + str(hyper_params['local_rank']) + '_example' + \
                                                       str(m) + '.nii'
                                    current_vol = np.squeeze(samples[m])
                                    nim = nib.Nifti1Image(current_vol, affine)
                                    nib.save(nim, os.path.join(hyper_params['samples_folder'], current_filename))
                                ...
                                current_dir = [os.path.join(hyper_params['samples_folder'], 'rank' +
                                                            str(hyper_params['local_rank']) + '_' + str(n)) for n in
                                ...
                                visuals.image_grid(samples, epoch, hyper_params['recon_folder'],
                                                   temp_prefix + 'samples_rank' + '_' +
                                                   str(hyper_params['local_rank']), 5,
                                                   norm_recons=True)
                                if hyper_params['likelihood'] == 'Gaussian' and hyper_params['predict_x_var']:
                                    visuals.image_grid(samples_std, epoch, hyper_params['recon_folder'],
                                                       temp_prefix + 'samples_stds_rank' + '_' +
                                                       str(hyper_params['local_rank']), 5,
                                                       norm_recons=True)
