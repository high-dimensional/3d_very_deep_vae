...
def create_data_transformations(
    hyper_params, device, keys=None, clamp_percentiles=None
):
    """
    ...
    misc.print_0(
        hyper_params,
        "Clamping input outside percentiles "
        + str(clamp_percentiles[0])
        + " and "
    ...
    if "veto_transformations" in hyper_params and hyper_params["veto_transformations"]:
        ...
        train_transforms = [
            monai_trans.LoadImaged(keys=keys),
            monai_trans.AddChanneld(keys=keys),
        ]

        if "crop_nii_at_loadtime" in hyper_params:
            print(
                "Cropping an padding on the basis of the volume occupied by Biobank T1s at 181x217x181"
            )
            train_transforms += [
                CropNIIByGivenAmount(
                    keys=keys, crop_ranges=hyper_params["crop_nii_at_loadtime"]
                ),
        ...
        if "crop_nii_at_loadtime" in hyper_params:
            print(
                "Cropping an padding on the basis of the voplume ocupied by Biobank T1s at 181x217x181"
            )
            train_transforms += [
                CropNIIByGivenAmount(
                    keys=keys, crop_ranges=hyper_params["crop_nii_at_loadtime"]
        ...
        train_transforms += [
            Anisotropiser(
                keys=keys, scale_range=anisotroper_ranges, prob=prob_anisotroper
            ),
        ...
        train_transforms += [
            monai_trans.RandHistogramShiftd(
                keys=keys,
                num_control_points=histogram_shift_control_points,
                prob=prob_torchvision_histogram,
            ),
            monai_trans.RandScaleIntensityd(
                keys=keys, factors=0.05, prob=prob_torchvision_simple
            ),
        ...
        train_transforms += [
            ...
            TorchIOWrapper(
                keys=keys,
                p=prob_spiking,
                trans=torchio_trans.RandomSpike(intensity=(1, 2), p=1),
            ),
            monai_trans.ToTensord(keys=keys),
        ]

        train_transforms += [
            monai_trans.RandAffined(
                keys=keys,
                prob=prob_affine,
                ...
                padding_mode="border",
                device=device
                if misc.key_is_true(hyper_params, "affine_and_elastic_on_gpu")
                else None,
                as_tensor_output=False,
            ),
            monai_trans.RandSpatialCropd(
                keys=keys,
                roi_size=min_small_crop_size,
                random_center=True,
                random_size=False,
            ),
            monai_trans.Rand3DElasticd(
                keys=keys,
        ...
        if misc.key_is_true(hyper_params, "apply_augmentations_to_validation_set"):
            misc.print_0(
                hyper_params, "Applying augmentation transformations to validation set"
            )
