...
def create_data_transformations_2d(
    hyper_params,
    device,
    keys=None,
    clamp_percentiles=None,
    keys_json=None,
    keys_imaging=None,
):
    """
    ...
    train_transforms += [
        ClampByPercentile(
            keys=["x0", "x1"], lower=clamp_percentiles[0], upper=clamp_percentiles[1]
        )
    ]
    train_transforms += [monai_trans.NormalizeIntensityd(keys=["x0", "x1"])]
    if misc.key_is_true(hyper_params, "use_tanh_output"):
        train_transforms += [
            monai_trans.ScaleIntensityd(["x0", "x1"], minv=-1.0, maxv=1.0)
        ]
    elif misc.key_is_true(hyper_params, "use_sigmoid_output"):
        train_transforms += [
            monai_trans.ScaleIntensityd(["x0", "x1"], minv=0, maxv=1.0)
        ]
    ...
    if misc.key_is_true(hyper_params, "simulate_missingness"):
        train_transforms += [
            SimulateMissingnessAugmentation(keys=keys, keys_imaging=keys_imaging)
        ]
