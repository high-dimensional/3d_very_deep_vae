...
class LeaveRandomCube(Randomizable, MapTransform):
    ...
    def randomize(self, data: Optional[Any] = None) -> None:
        self.side_lengths = [self.R.randint(low=self.min_cube_size[0], high=self.vol_shape[0]),
                             self.R.randint(low=self.min_cube_size[1], high=self.vol_shape[1]),
                             self.R.randint(low=self.min_cube_size[2], high=self.vol_shape[2])]
    ...
    def __call__(self, data: Mapping[Hashable, np.ndarray]) -> Dict[Hashable, np.ndarray]:
        ...
        for key in self.keys:
            ...
            mask[0, self.offsets[0]:self.offsets[0] + self.side_lengths[0],
                    ...
                    self.offsets[2]:self.offsets[2] + self.side_lengths[2]] = 1
